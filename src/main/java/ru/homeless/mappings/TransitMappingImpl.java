...
public class TransitMappingImpl implements ICustomMappingWordDocument {
    ...
    public WordprocessingMLPackage getDocument(Map map, Client client) {
        return new DocTypeProcessor(IDocumentMapping.DOCUMENT_TRANSIT_TEMPLATE_PATH).replaceParametersInDocument(map, Util.attachPhoto(client, log), ICustomMappingWordDocument.AVATAR_LOCATION_BOTTOM_CENTER);
    }
